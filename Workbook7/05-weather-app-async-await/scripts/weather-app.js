...
// must add "async" to the function definition to use await
async function searchCities()
{
    const searchValue = citySearchInput.value

    // use the city service to search for cities
    let cities = await cityService.search(searchValue)
...
async function getForecast(event){
    ...
    const forecast = await weatherService.getForecastByCoordinates(data.lat, data.lng)
